...
namespace Runtime.Commands.Stack
{
    public class StackMoverCommand
    {
        ...
        public void Execute(float directionX,float directionY, List<GameObject> collectableStack)
        {
            ...
            float movementFactor = Mathf.InverseLerp(-9, 9, collectableStack[0].transform.localPosition.x - directionX);
            float rotationY = Mathf.Lerp( 180f + directionX, -180f + directionX,movementFactor);  
             collectableStack[0].transform.localRotation = Quaternion.Euler(0f,180+ rotationY, 0f);
            StackItemsLerpMove(collectableStack);
            StackItemsLerpMovee(directionX,collectableStack);
        ...
        public void StackItemsLerpMovee(float directionX, List<GameObject> collectableStack)
        {
            for (int i = 1; i < collectableStack.Count; i++)
            {
                float ii = i + 10;
                // -15f,15f
                float movementFactor = Mathf.InverseLerp(-ii, ii, collectableStack[i].transform.localPosition.x - directionX);
                float rotationY = Mathf.Lerp( 180f + directionX, -180f + directionX,movementFactor);  
 
                collectableStack[i].transform.localRotation = Quaternion.Euler(0f,180+ rotationY, 0f);
            }
        }
